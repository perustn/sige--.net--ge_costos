...
Public Class FrmGuiasAsociadas_PreTenido
    ...
    Sub CARGA_CABECERA()
        Try
            strSQL = String.Empty
            strSQL &= vbNewLine & "EXEC SM_MUESTRA_GUIAS_DE_FACTURA_TX_MOVISTK_PRE_TENIDO"
    ...
    Sub CARGA_DETALLE(ByVal AlmacenSel As String, ByVal MovimientoSel As String)
        Try
            If Trim(AlmacenSel) <> "" Then
                strSQL = String.Empty
                strSQL &= vbNewLine & "EXEC SM_MUESTRA_GUIAS_POR_FACTURAR_DETALLE_PRETENIDO"
    ...
    Private Sub ButtonBar1_ItemClick(sender As Object, e As Janus.Windows.ButtonBar.ItemEventArgs) Handles ButtonBar1.ItemClick
        Try
            Select Case e.Item.Key
                Case "ASIGNAR"
                    Using oAsig As New FrmGuiasAsociadas_PreTenido_AsigGuias
                        With oAsig
    ...
    Sub DESASIGNA()
        Try
            strSQL = String.Empty
            strSQL &= vbNewLine & "EXEC UP_DESASIGNA_GUIA_FACTURA_TX_MOVISTK_PRE_TENIDO"
    ...
    Sub CompletarDatos()
        Try
            ...
            For i = 1 To gexDetalle.RowCount
                ...
                strSQL = String.Empty
                strSQL &= vbNewLine & "EXEC UP_ACTUALIZA_CN_DATOSCOMPLGUIA_FARDOS_TX_MOVISTK_PRE_TENIDO"
                strSQL &= vbNewLine & String.Format(" @COD_ALMACEN          = '{0}'", gexCabecera.GetValue(gexCabecera.RootTable.Columns("Cod_almacen").Index))
                strSQL &= vbNewLine & String.Format(",@NUM_MOVSTK           = '{0}'", gexCabecera.GetValue(gexCabecera.RootTable.Columns("Movimiento").Index))
                strSQL &= vbNewLine & String.Format(",@IMPORTE_FACTURA      = '{0}'", gexDetalle.GetValue(gexDetalle.RootTable.Columns("Imp_Valorizado").Index))
    ...
    Private Sub gexDetalle_EditingCell(sender As Object, e As EditingCellEventArgs) Handles gexDetalle.EditingCell
        If e.Column.Index <> gexDetalle.RootTable.Columns("IMP_valorizado").Index Then
            e.Cancel = True
        Else
            iNro_Servicio = "1"
        End If
