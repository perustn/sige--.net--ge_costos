...
#End If
    ...
    '        Return Nothing   'CAR  DECIA result
    '    End Try
    'End Function
    ...
    '    Sub BuscaCampo(ByRef pRs_Lista As ADODB.Recordset, ByRef pCampo As String, ByRef pValor As String)
    '        On Error GoTo hand
    '        Dim pIndice As Short
    '        Dim pRs_Prov As New ADODB.Recordset

    '        If Not pRs_Lista.EOF And Not pRs_Lista.BOF Then
    '            pRs_Prov = pRs_Lista.Clone
    '            pIndice = 0
    '            pRs_Prov.MoveFirst()
    '            While Not pRs_Prov.EOF
    '                If UCase(Mid(pRs_Prov.Fields(pCampo).Value, 1, Len(pValor))) = pValor Then
    '                    pRs_Lista.MoveFirst()
    '                    pRs_Lista.Move((pIndice))

    '                    pRs_Prov.Close()
    '                    'UPGRADE_NOTE: El objeto pRs_Prov no se puede destruir hasta que no se realice la recolección de los elementos no utilizados. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
    '                    pRs_Prov = Nothing
    ...
    '            End While
    '        End If
    '        If pRs_Lista.RecordCount > 0 Then
    '            pRs_Prov.Close()
    '        End If
    '        'UPGRADE_NOTE: El objeto pRs_Prov no se puede destruir hasta que no se realice la recolección de los elementos no utilizados. Haga clic aquí para obtener más información: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6E35BFF6-CD74-4B09-9689-3E1A43DF8969"'
    '        pRs_Prov = Nothing
    ...
    Public Sub ImpresionCIP(Opcion As String, ID As String, ListaIds As String)
        ...
        strSQL = "CF_ORDPROCOLTAL_PRENDAS_REIMPRIME '" & Opcion & "','" & ID & "','" & ListaIds & "'"
        ...
        End If
    End Sub
    Public Sub Imprime_ICA_ZPL_STN(NUM_CAJA As String)
        'CREAMOS EL ARCHIVO DONDE GUARDAREMOS EL CODIFO ZPL
        Const ArchivoCips As String = "C:\TEMP\SIP.txt"
        ...
        strSQL = "ETIQUETA_CAJA_SALDO_CMT '" & NUM_CAJA & "','" & vusu & "','C'"
        ...
        For Each Row As DataRow In Dt.Rows
            ...
            strSQL = "[ETIQUETA_CAJA_PRODUCCION_CMT_DETALLE_SALDOS] 0 ," & CStr(NUM_CAJA)
        ...
        Call ImprimirArchivo(ArchivoCips) 'LLAMAMOS AL METODO DE IMPRESION
    End Sub
    Public Sub Imprime_ICA_ZEBRA_CMT_PRODUCCION(NUM_CAJA As String)
        ...
        strSQL = "EXEC ETIQUETA_CAJA_PRODUCCION_TDV '" & NUM_CAJA & "'"
        ...
        For Each Row As DataRow In Dt.Rows
            ...
            strSQL = "ETIQUETA_CAJA_PRODUCCION_CMT_DETALLE '" & Row("Num_Packing") & "','" & CStr(NUM_CAJA) & "'"
            Dim Dt1 As DataTable = oHP.DevuelveDatos(strSQL, cconnect)
    ...
    Public Sub GeneraInterfase_Cn_Docum_New(ByVal sNum_Corre As String, flg_Status As String, strOpcion As String)
        'On Error GoTo errx
        '
        'Dim SSQL As String
        '
        'SSQL = GeneraCadena(sPrefijoTablaSLIM & "_DOCUM", "SM_CN_DOCUM '" & sNum_corre & "','" & flg_Status & "'", cCONNECT, strOpcion, 1)
        '
        'SLIMExecuteSQL sSLIMConnect, SSQL
        '
        'Exit Sub
        'errx:
        '    sDescError = err.Description
        '    errores err.Number
        '    ExecuteSQL cCONNECT, "Cn_Man__Errores_Interfase_Slim '020" & Soption & "','','','" & sNum_corre & "','','" & vusu & "','" & ComputerName & "','" & sDescError & "'"

    End Sub

    Public Sub GeneraInterfase_GENERAVOUCHERCABECERA(ByVal sNum_Corre As String, flg_Status As String)
        'On Error GoTo errx
        '
        'Dim sSql As String
        '
        ''Exit Sub
        'EjecutaCadenaSLIM sPrefijoTablaASIENTOSLIM & "ASIENTOS", "SM_MUESTRA_VOUCHER_CABECERA_PARA_TRANSFERENCIA '" & sNum_Corre & "'", cCONNECT, "I", 1
        '
        '
        'Exit Sub
        'errx:
        '    sDescError = err.Description
        '    errores err
        '    ExecuteSQL cCONNECT, "Cn_Man__Errores_Interfase_Slim '070" & Soption & "','','','" & sNum_Corre & "','','" & vusu & "','" & ComputerName & "','" & sDescError & "'"

    End Sub

    Public Sub GeneraInterfase_GENERAVOUCHERDETALLE(ByVal sNum_Corre As String, flg_Status As String)
        'On Error GoTo errx
        '
        'Dim sSql As String
        '
        ''Exit Sub
        'EjecutaCadenaSLIM sPrefijoTablaASIENTOSLIM & "ASIENTOS_DET", "SM_MUESTRA_VOUCHER_DETALE_PARA_TRANSFERENCIA '" & sNum_Corre & "'", cCONNECT, "I", 1
        '
        'Exit Sub
        'errx:
        '    sDescError = err.Description
        '    errores err
        '    ExecuteSQL cCONNECT, "Cn_Man__Errores_Interfase_Slim '080" & Soption & "','','','" & sNum_Corre & "','','" & vusu & "','" & ComputerName & "','" & sDescError & "'"

    End Sub

    Public Sub GeneraInterfase_REVIERTEVOUCHERCABECERA(ByVal sNum_Corre As String, flg_Status As String)
        'On Error GoTo errx
        '
        'Dim sSql As String
        '
        ''Exit Sub
        '
        'EjecutaCadenaSLIM sPrefijoTablaASIENTOSLIM & "ASIENTOS", "SM_MUESTRA_VOUCHER_CABECERA_PARA_TRANSFERENCIA '" & sNum_Corre & "'", cCONNECT, "D", 1
        '
        '
        'Exit Sub
        'errx:
    ...
    Public Sub GeneraInterfase_REVIERTEVOUCHERDETALLE(ByVal sNum_Corre As String, flg_Status As String)
        'On Error GoTo errx
        '
        'Dim sSql As String
        ...
        ''Exit Sub
        '
        'Set mRS = GetRecordset(cCONNECT, "SM_MUESTRA_VOUCHER_CABECERA_PARA_TRANSFERENCIA '" & sNum_Corre & "'")
        ...
        '        Set mRS = Nothing
        '
        '        'sSql = "DELETE FROM " & sPrefijoTablaASIENTOSLIM & "ASIENTOS_DET WHERE SUBSTRING(CODIGO,1,15) LIKE  '" & sCodigo & "%'"    '200411*60*00010'
        '        sSql = "DELETE FROM " & sPrefijoTablaASIENTOSLIM & "ASIENTOS_DET WHERE CODIGO LIKE  '" & sCodigo & "%'"
        '
        '        SLIMExecuteSQL sSLIMConnect, sSql
        '    Else
        '        Debug.Print sNum_Corre
        '    End If
        'Else
        '    ExecuteSQL cCONNECT, "Cn_Man__Errores_Interfase_Slim '080" & Soption & "','','','" & sNum_Corre & "','','" & vusu & "','" & ComputerName & "','" & "NO SE ENCONTRO NUM_CORRE EN  SM_MUESTRA_VOUCHER_CABECERA_PARA_TRANSFERENCIA " & "'"
        '    Exit Sub
        'End If
        '
        'Exit Sub
        'errx:
        '    ExecuteSQL cCONNECT, "Cn_Man__Errores_Interfase_Slim '080" & Soption & "','','','" & sNum_Corre & "','','" & vusu & "','" & ComputerName & "','" & err.Description & "'"
        '    errores err
    End Sub
